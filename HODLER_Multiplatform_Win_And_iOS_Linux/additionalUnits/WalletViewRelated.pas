...
uses
  ...
  System.Generics.Collections, System.character, System.DateUtils, System.Messaging,
  System.Variants, System.IOUtils, FMX.Types, FMX.Graphics, FMX.Controls, FMX.Forms,
  FMX.Dialogs, FMX.StdCtrls, FMX.Controls.Presentation, FMX.Styles, System.ImageList,
  FMX.ImgList, FMX.Ani, FMX.Layouts, FMX.ExtCtrls, Velthuis.BigIntegers, FMX.ScrollBox,
  FMX.Memo, FMX.platform, System.Threading, Math, DelphiZXingQRCode, FMX.TabControl,
  FMX.Edit, FMX.Clipboard, FMX.VirtualKeyBoard, JSON, languages,
  WalletStructureData, FMX.Media, FMX.Objects, uEncryptedZipFile, System.Zip
{$IFDEF ANDROID}, FMX.VirtualKeyBoard.Android, Androidapi.JNI, Androidapi.JNI.GraphicsContentViewText,
  Androidapi.JNI.App, Androidapi.JNI.JavaTypes, Androidapi.Helpers, FMX.platform.Android,
  Androidapi.JNI.Provider, Androidapi.JNI.Net, Androidapi.JNI.WebKit, Androidapi.JNI.Os,
  Androidapi.NativeActivity, Androidapi.JNIBridge, SystemApp
{$ENDIF},
  {FMX.Menus,}
ZXing.BarcodeFormat, ZXing.ReadResult, ZXing.ScanManager, FMX.EditBox, FMX.SpinBox,
FMX.Gestures, FMX.Effects, FMX.Filter.Effects, System.Actions, FMX.ActnList,
System.Math.Vectors, FMX.Controls3D, FMX.Layers3D, FMX.StdActns, FMX.MediaLibrary.Actions,
FMX.ComboEdit;
...
procedure PrepareSendTabAndSend(from: TWalletInfo; sendto: AnsiString; Amount, Fee: BigInteger; MasterSeed: AnsiString; coin: AnsiString = 'bitcoin');
...
uses
  uHome, misc, AccountData, base58, bech32, CurrencyConverter, SyncThr, WIF,
  Bitcoin, coinData, cryptoCurrencyData, Ethereum, secp256k1, tokenData,
  transactions, AccountRelated, TCopyableEditData, BackupRelated, KeypoolRelated;
...
begin
  ...
  begin
    ...
    popupWindow.Create(removeSpace(TfmxObject(Sender).Parent.TagString) + ' ' + dictionary('CopiedToClipboard'));
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      popupWindow.Create(removeSpace(Tedit(TfmxObject(Sender).Parent).Text) + ' ' + dictionary('CopiedToClipboard'));
      exit;
    end;
    if TfmxObject(Sender).Parent is TButton then
    begin
      svc.setClipboard(removeSpace(Tedit(TfmxObject(Sender).Parent.Parent).Text));
      popupWindow.Create(removeSpace(Tedit(TfmxObject(Sender).Parent.Parent).Text) + ' ' + dictionary('CopiedToClipboard'));
    end;
...
begin
  ...
  with frmhome do
    ...
    begin
      wvFee.Text := BigIntegertoFloatStr(100000 * length(currentAccount.aggregateUTXO(CurrentCoin)), CurrentCoin.decimals);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        WVRealCurrency.Text := floatToStrF(CurrencyConverter.calculate(strToFloatDef(lbBalanceLong.Text, 0) * CurrentCryptoCurrency.rate), ffFixed, 18, 2);
...
begin
  ...
  begin
    ...
    begin
      ...
      lblFee.Text := wvFee.Text + ' ' + floatToStrF(CurrencyConverter.calculate(strToFloatDef(wvFee.Text, 0) * 66666 * CurrentCryptoCurrency.rate / (1000000.0 * 1000000.0 * 1000000.0)), ffFixed, 18, 6) + ' ' + CurrencyConverter.symbol;
    end
    else if isEthereum then
    begin
      lblFeeHeader.Text := dictionary('GasPriceWEI') + ': ';
      lblFee.Text := wvFee.Text + ' ' + AvailableCoin[CurrentCoin.coin].shortcut + ' = ' + floatToStrF(CurrencyConverter.calculate(strToFloatDef(wvFee.Text, 0) * CurrentCoin.rate * 21000 / (1000000.0 * 1000000.0 * 1000000.0)), ffFixed, 18, 6) + ' ' + CurrencyConverter.symbol;
    end
    else
    begin
      ...
      satb := BigInteger(StrFloatToBigInteger(wvFee.Text, CurrentCryptoCurrency.decimals) div curWU).asInteger;
      ...
      lblFee.Text := wvFee.Text + ' (' + intToStr(satb) + curWUstr + AvailableCoin[CurrentCoin.coin].shortcut + ' = ' + floatToStrF(CurrencyConverter.calculate(strToFloatDef(wvFee.Text, 0) * CurrentCoin.rate), ffFixed, 18, 6) + ' ' + CurrencyConverter.symbol;
    end;
...
begin
  ...
  begin
    ...
    begin
      a := ((180 * length(currentAccount.aggregateUTXO(CurrentCoin)) + (34 * 2) + 12));
      curWU := a.asInteger;
      a := (a * StrFloatToBigInteger(CurrentCoin.efee[round(FeeSpin.Value) - 1], CurrentCoin.decimals)) div 1024;
      if (CurrentCoin.coin = 0) or (CurrentCoin.coin = 1) then
        a := a * 4;
      a := Max(a.asInteger, 500);
      wvFee.Text := BigIntegertoFloatStr(a, CurrentCoin.decimals);
      // CurrentCoin.efee[round(FeeSpin.Value) - 1] ;
      lblBlockInfo.Text := dictionary('ConfirmInNext') + ' ' + intToStr(round(FeeSpin.Value)) + ' ' + dictionary('Blocks');
    end
    else
      FeeSpin.Value := 1.0;
  end;
...
begin
  ...
  begin
    ...
    if not isEthereum then
      Fee := StrFloatToBigInteger(wvFee.Text, AvailableCoin[CurrentCoin.coin].decimals)
    else
    begin
      if isTokenTransfer then
    ...
    begin
      Amount := StrFloatToBigInteger(wvAmount.Text, AvailableCoin[CurrentCoin.coin].decimals);
    ...
    if (isEthereum) and (isTokenTransfer) then
      Amount := StrFloatToBigInteger(wvAmount.Text, CurrentCryptoCurrency.decimals);
    ...
    if { (not isEthereum) and } (not isTokenTransfer) then
      if Amount + Fee > (currentAccount.aggregateBalances(CurrentCoin).confirmed) then
      begin
        popupWindow.Create(dictionary('AmountExceed'));
        exit;
      end;
    Address := removeSpace(WVsendTO.Text);
    if (CurrentCryptoCurrency is TWalletInfo) and (TWalletInfo(CurrentCryptoCurrency).coin in [3, 7]) and isCashAddress(Address) then
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          adrLabel.Text := AvailableCoin[wd.coin].displayName + ' (' + AvailableCoin[wd.coin].shortcut + ')';
        end
        else
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // begin
          if not (frmhome.HexPrivKeyCompressedRadioButton.IsChecked or frmhome.HexPrivKeyNotCompressedRadioButton.IsChecked) then
            exit;
      ...
      begin
        ...
        wd := TWalletInfo.Create(newcoinID, -1, -1, Ethereum_PublicAddrToWallet(pub), frmhome.CoinPrivKeyDescriptionEdit.Text);
        wd.pub := pub;
      ...
      begin
        wd := TWalletInfo.Create(newcoinID, -1, -1, Bitcoin_PublicAddrToWallet(pub, AvailableCoin[newcoinID].p2pk), frmhome.CoinPrivKeyDescriptionEdit.Text);
        wd.pub := pub;
        wd.EncryptedPrivKey := speckEncrypt((TCA(MasterSeed)), out);
        wd.isCompressed := isCompressed;
...
begin
  ...
  walletInfo := coinData.createCoin(newcoinID, newID, 0, MasterSeed, frmhome.NewCoinDescriptionEdit.Text);
...
begin
  ...
  lastHistCC := 10;
//  if not TfmxObject(Sender).tag = $deadbeef then
...
//  else
 //   CurrentCryptoCurrency := TWalletInfo(TfmxObject(Sender).TagObject);
  frmhome.InstantSendLayout.Visible := TWalletInfo(CurrentCryptoCurrency).coin = 2;
...
{$ENDIF}
    ...
    WVRealCurrency.Text := floatToStrF(strToFloatDef(wvAmount.Text, 0) * CurrentCryptoCurrency.rate, ffFixed, 15, 2);
...
{$ENDIF}
    ...
    begin
      ...
      if (TWalletInfo(CurrentCryptoCurrency).x = -1) and (TWalletInfo(CurrentCryptoCurrency).Y = -1) and (TWalletInfo(CurrentCryptoCurrency).isCompressed = false) then
        AddressTypelayout.Visible := false;
    ...
    begin
      ...
      a := ((180 * length(TWalletInfo(CurrentCryptoCurrency).UTXO) + (34 * 2) + 12));
      curWU := a.asInteger;
      a := (a * StrFloatToBigInteger(CurrentCoin.efee[round(FeeSpin.Value) - 1], CurrentCoin.decimals)) div 1024;
      if (CurrentCoin.coin = 0) or (CurrentCoin.coin = 1) then
        a := a * 4;
...
begin
  ...
  begin
    ...
    if TWalletInfo(CurrentCryptoCurrency).coin in [3, 7] then
      frmhome.wvAddress.Text := bitcoinCashAddressToCashAddress(CurrentCryptoCurrency.addr)
    else
      frmhome.wvAddress.Text := CurrentCryptoCurrency.addr;

    if (TWalletInfo(CurrentCryptoCurrency).coin <> 4) and (not isTokenTransfer) then
    begin
      sumConfirmed := 0;
      sumUnconfirmed := 0;
      SumFiat := 0;
      ...
      lbBalance.Text := BigIntegerBeautifulStr(currentAccount.aggregateBalances(CurrentCoin).confirmed, CurrentCryptoCurrency.decimals);
      lbBalanceLong.Text := BigIntegertoFloatStr(currentAccount.aggregateBalances(CurrentCoin).confirmed, CurrentCryptoCurrency.decimals);
      lblFiat.Text := floatToStrF(currentAccount.aggregateFiats(CurrentCoin), ffFixed, 15, 2);

      TopInfoConfirmedValue.Text := ' ' + BigIntegertoFloatStr(currentAccount.aggregateBalances(CurrentCoin).confirmed, CurrentCryptoCurrency.decimals) + ' ' + CurrentCryptoCurrency.shortcut;
      ...
      TopInfoUnconfirmedValue.Text := ' ' + BigIntegertoFloatStr(currentAccount.aggregateBalances(CurrentCoin).unconfirmed, CurrentCryptoCurrency.decimals) + ' ' + CurrentCryptoCurrency.shortcut;
      ...
      ShortcutFiatLabel.Text := floatToStrF(currentAccount.aggregateFiats(CurrentCoin), ffFixed, 15, 2);
    end
    else
    begin
      lbBalance.Text := BigIntegerBeautifulStr(CurrentCryptoCurrency.confirmed, CurrentCryptoCurrency.decimals);

      lbBalanceLong.Text := BigIntegertoFloatStr(CurrentCryptoCurrency.confirmed, CurrentCryptoCurrency.decimals);

      lblFiat.Text := floatToStrF(CurrentCryptoCurrency.getFiat(), ffFixed, 15, 2);

      TopInfoConfirmedValue.Text := ' ' + BigIntegertoFloatStr(CurrentCryptoCurrency.confirmed, CurrentCryptoCurrency.decimals) + ' ' + CurrentCryptoCurrency.shortcut;
      TopInfoConfirmedFiatLabel.Text := floatToStrF(CurrentCryptoCurrency.getConfirmedFiat, ffFixed, 15, 2) + ' ' + CurrencyConverter.symbol;

      TopInfoUnconfirmedValue.Text := ' ' + BigIntegertoFloatStr(CurrentCryptoCurrency.unconfirmed, CurrentCryptoCurrency.decimals) + ' ' + CurrentCryptoCurrency.shortcut;
      ...
      ShortcutFiatLabel.Text := floatToStrF(CurrentCryptoCurrency.getFiat(), ffFixed, 15, 2);
    end;
    ...
    TopInfoUnconfirmedFiatLabel.Width := Max(TopInfoUnconfirmedFiatLabel.Canvas.TextWidth(TopInfoUnconfirmedFiatLabel.Text) * TopInfoUnconfirmedFiatLabel.Font.Size / 12, TopInfoConfirmedFiatLabel.Canvas.TextWidth(TopInfoConfirmedFiatLabel.Text) * TopInfoConfirmedFiatLabel.Font.Size / 12) + 6;
...
begin
  ...
  begin
    ...
    if (CurrentCryptoCurrency is TWalletInfo) and (TWalletInfo(CurrentCryptoCurrency).coin in [3, 7]) then
    begin
    ...
    PrepareSendTabAndSend(CurrentCoin, Address, Amount, Fee, MasterSeed, AvailableCoin[CurrentCoin.coin].Name);
...
begin
  ...
  begin
    ...
    if (t.addr = walletAddressForNewToken) and (t.id = (Tcomponent(Sender).Tag + 10000)) then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      cryptoCurrency(fmxObj.TagObject).orderInWallet := round(TPanel(fmxObj).Position.Y);
    end;
...
begin
  ...
  begin
    ...
    loadDictionary(loadLanguageFile(TPopupBox(Sender).Items[TPopupBox(Sender).ItemIndex]));
      ...
      begin
        ...
        begin
          ...
          case PopupBox1.ItemIndex of
            ...
            0:
              if (compLex(cryptoCurrency(OrganizeList.Content.Children[i].TagObject), cryptoCurrency(OrganizeList.Content.Children[j].TagObject)) > 0) and (TPanel(OrganizeList.Content.Children[i]).Position.Y < TPanel(OrganizeList.Content.Children[j]).Position.Y) then
                swapFlag := true;

            1:
              if (compVal(cryptoCurrency(OrganizeList.Content.Children[i].TagObject), cryptoCurrency(OrganizeList.Content.Children[j].TagObject)) > 0) and (TPanel(OrganizeList.Content.Children[i]).Position.Y > TPanel(OrganizeList.Content.Children[j]).Position.Y) then
                swapFlag := true;

            2:
              if (compAmount(cryptoCurrency(OrganizeList.Content.Children[i].TagObject), cryptoCurrency(OrganizeList.Content.Children[j].TagObject)) > 0) and (TPanel(OrganizeList.Content.Children[i]).Position.Y > TPanel(OrganizeList.Content.Children[j]).Position.Y) then
                swapFlag := true;
          ...
          begin
            ...
            TPanel(OrganizeList.Content.Children[i]).Position.Y := TPanel(OrganizeList.Content.Children[j]).Position.Y - 1;
            TPanel(OrganizeList.Content.Children[j]).Position.Y := temp - 1;

            if (i = 8) and (j = 11) then
            begin
              compLex(cryptoCurrency(OrganizeList.Content.Children[i].TagObject), cryptoCurrency(OrganizeList.Content.Children[j].TagObject));
            end;
...
begin
  ...
  begin
    ...
    HistoryTransactionValue.Text := BigIntegertoFloatStr(th.CountValues, CurrentCryptoCurrency.decimals);
    ...
    HistoryTransactionDate.Text := FormatDateTime('dd mmm yyyy hh:mm', UnixToDateTime(strToIntdef(th.Data, 0)));
    ...
    begin
      ...
      if LeftStr(fmxObject.Name, length('HistoryValueAddressPanel_')) = 'HistoryValueAddressPanel_' then
      begin
        fmxObject.DisposeOf;
        i := 0;
      end;
...
{$IFDEF ANDRIOD}
      ...
      Panel.Touch.InteractiveGestures := [TInteractiveGesture.DoubleTap, TInteractiveGesture.LongTap];
{$ENDIF}
      ...
      valuelbl.Text := BigIntegertoFloatStr(th.values[i], CurrentCryptoCurrency.decimals);
...
{$IFDEF ANDRIOD}
      ...
      valuelbl.Touch.InteractiveGestures := [TInteractiveGesture.DoubleTap, TInteractiveGesture.LongTap];
...
begin
  ...
  begin
    ...
    if (Panel.TagObject is TWalletInfo) and (TWalletInfo(Panel.TagObject).coin <> 4) then
    begin

      wdArray := currentAccount.getWalletWithX(TWalletInfo(Panel.TagObject).x, TWalletInfo(Panel.TagObject).coin);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            popupWindow.Create('You must check whether your hey is compressed or not');
            exit;
          end;
          ...
          begin
            ...
            wd := TWalletInfo.Create(newcoinID, -1, -1, Bitcoin_PublicAddrToWallet(comkey, AvailableCoin[newcoinID].p2pk), 'Imported');
            wd.pub := comkey;
            request := HODLER_URL + 'getSegwitBalance.php?coin=' + AvailableCoin[wd.coin].Name + '&' + segwitParameters(wd);
            WData := getDataOverHTTP(request);
            ts := TStringList.Create();
            ts.Text := WData;
            if strToFloatDef(ts[0], 0) + strToFloatDef(ts[1], 0) = 0 then
            begin
              WData := getDataOverHTTP(HODLER_URL + 'getSegwitHistory.php?coin=' + AvailableCoin[wd.coin].Name + '&' + segwitParameters(wd));
            ...
            wd := TWalletInfo.Create(newcoinID, -1, -1, Bitcoin_PublicAddrToWallet(notkey, AvailableCoin[newcoinID].p2pk), '');
            wd.pub := comkey;

            WData := getDataOverHTTP(HODLER_URL + 'getBalance.php?coin=' + AvailableCoin[wd.coin].Name + '&address=' + wd.addr);
            ts := TStringList.Create();
            ts.Text := WData;

            if strToFloatDef(ts[0], 0) + strToFloatDef(ts[1], 0) = 0 then
            begin
              WData := getDataOverHTTP(HODLER_URL + 'getHistory.php?coin=' + AvailableCoin[wd.coin].Name + '&address=' + wd.addr);
              if length(WData) > 10 then
              begin
                TThread.Synchronize(nil,
                  procedure
          ...
          begin
            ...
            wd := TWalletInfo.Create(newcoinID, -1, -1, Ethereum_PublicAddrToWallet(comkey), 'Imported');
            wd.pub := comkey;
